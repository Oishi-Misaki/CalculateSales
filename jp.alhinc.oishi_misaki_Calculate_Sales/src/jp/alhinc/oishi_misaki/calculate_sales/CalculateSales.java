...
public class CalculateSales {
	public static void main(String[] args) {
		...
		try {
			...
			// 売上ファイルの読込・加算
			if (!readBranchSaleFileAndSum(serchedNumsFile, branchCodeAndNameMap, commodityCodeAndNameMap,
					branchTatalSaleMap, commodityTatalSaleMap)) {
				return;
			}
			// ファイルの書出し
	 ...
	 * @param File : 支店定義ファイル
	 * 
	 * @return HasMap<String, String>:支店コード、支店名 ＠error ファイルが存在しない
	 * 
	 * @error 不正フォーマット
	 * 
	 * @error 予期せぬエラー
	...
	/*
	 * 支店定義/商品定義のマップを元に支店別商品別売上合計マップの作成 branchCodeAndName －＞
	 * createBranchTatalSaleMap commodityCodeAndNameMap －＞ commodityTatalSale
	 */
	 ...
	 * 
	 * @return HasMap<String, String>:商品コード、商品名 ＠error ファイルが存在しない
	 * 
	 * @error 不正フォーマット
	 * 
	 * @error 予期せぬエラー
	...
	public static HashMap<String, String> readCommodityDefinitionFile(File commodityDefinitionFile) throws IOException {
		...
		try {
			...
			while ((s = br.readLine()) != null) {
				String[] CodeAndName = s.split(",");
				if (CodeAndName.length != 2 || !CodeAndName[0].matches("^[0-9a-zA-Z]{8}$")
						|| CodeAndName[1].isEmpty()) {
	 ...
	 * 売上ファイル検索
	 * 
	 * @param ディレクトリ名 ＠return 検索結果のファイルList
	 */
	 ...
	 * @param 商品別売上合計
	 */
	public static boolean readBranchSaleFileAndSum(List<File> searchedFileList,
			HashMap<String, String> branchCodeAndNameMap, HashMap<String, String> commodityCodeAndNameMap,
		...
		for (File searchedFile : searchedFileList) {
			...
			try {
				...
				if (readLineSearchdFile.size() != 3 || !readLineSearchdFile.get(0).matches("^\\d{3}$")
						|| !readLineSearchdFile.get(1).matches("^[0-9a-zA-Z]{8}$")) {
				...
				Long branchTotalSaleSum = branchTatalSaleMap.get(readLineSearchdFile.get(0))
						+ Long.parseLong(readLineSearchdFile.get(2));
				...
				Long commodityTotalSaleSum = branchTatalSaleMap.get(readLineSearchdFile.get(0))
						+ Long.parseLong(readLineSearchdFile.get(2));
	...
	/*
	 * 売上ファイルの書出し readfile(String st, HashMap<String,String> fl,
	 * HashMap<String,Long> flsum)
	 * 
	 * @param filename
	 * 
	 * @param 定義ファイル
	 ...
	 * @error 予期せぬエラー
	 */
	public static boolean writeTotalSaleFile(File outfile, HashMap<String, String> codeAndName,
			HashMap<String, Long> TatalSaleMap) throws IOException {
		// sort
		...
		try {
			...
			for (Entry<String, Long> sortedToalSale : entries1) {
				bw.write(sortedToalSale.getKey() + "," + codeAndName.get(sortedToalSale.getKey()) + ","
						+ sortedToalSale.getValue().toString());
				bw.newLine();
			}
			return true;
		} catch (IOException e) {
